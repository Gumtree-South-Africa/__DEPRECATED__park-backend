...
public class GetFeedsV3CmdTest {
    ...
    @InjectMocks
    @Spy
    private GetFeedsV3Cmd cmd = new GetFeedsV3Cmd();
    ...
    public void givenNewFeedThenReturnKnownFeedActions() {
        ...
        Feed feedSoldAndItemForInterestedFollowers = new Feed();
        feedSoldAndItemForInterestedFollowers.setAction(NotificationAction.SOLD_AN_ITEM_FOR_INTERESTED_FOLLOWERS.toString());
        
        Feed fbFriendUsingTheApp = new Feed();
        fbFriendUsingTheApp.setAction(NotificationAction.FB_FRIEND_USING_THE_APP.toString());
        
        //when
        List<Feed> feeds = Arrays.asList(feedSoldAndItemForInterestedFollowers, fbFriendUsingTheApp);
        ...
        assertEquals(NotificationAction.SOLD_AN_ITEM.toString(), response.getFeeds().get(0).getAction());
        assertEquals(NotificationAction.FOLLOW_USER.toString(), response.getFeeds().get(1).getAction());
    }

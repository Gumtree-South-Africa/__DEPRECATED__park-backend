...
public class RemoveBlacklistedWordImplTest {

	@InjectMocks
	private RemoveBlacklistedWordImpl removeBlacklistedWordImpl = new RemoveBlacklistedWordImpl();
	...
	@Test
	public void givenNoExitsWordThenException() {
		when(blackListDao.findOne(request.getId())).thenReturn(null);
		try {
			removeBlacklistedWordImpl.execute(request);
			fail();
	...
	@Test
	public void givenExistingWordThenRemoveBlackListedWordSuccess() {
		BlackList word = Mockito.mock(BlackList.class);
		when(blackListDao.findOne(request.getId())).thenReturn(word);
		ServiceResponse response = removeBlacklistedWordImpl.execute(request);
